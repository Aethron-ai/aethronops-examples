...
| **Azure OpenAI / Cognitive Services** | AI model hosting (GPT, embeddings, vision, speech) — managed inference endpoint. | Managed Identity auth, custom subdomain, network ACLs in premium (MCSB DP-1/NS-3) |
| **Azure AI Search** | Full-text and vector search engine — indexes documents with AI enrichment. | Managed Identity, API key + RBAC auth, network isolation in premium (MCSB NS-3) |
| **Storage Account** | Object storage for data, logs, and artifacts. | TLS 1.2 enforced, encryption at rest (AES-256), blob soft-delete, network deny by default (MCSB DP-1) |
...
> **Important — AI Model Region Availability**
>
> Azure AI models (GPT-4o, GPT-4, Whisper, DALL-E, embeddings...) are NOT available
> in all regions. Before deploying, check model availability for your target region:
> - [Azure OpenAI model availability](https://learn.microsoft.com/azure/ai-services/openai/concepts/models#model-summary-table-and-region-availability)
> - Recommended regions for full model support: `swedencentral`, `eastus`, `eastus2`, `westus3`
> - `westeurope` and `francecentral` have limited model availability
>
> To change region, update `location` and `region_short` in your tfvars file.
